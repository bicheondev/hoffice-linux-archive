...
int main(int argc, char **argv) {
    ...
    if (program.interp_path[0] == '\0') {
        errno = 0;
        fatal("M5 program does not contain PT_INTERP");
    }
    ...
    fprintf(stderr,
            "hrt-m5: program=[%p,%p) entry=%p syscalls=%zu fs=%zu "
            "interp=%s\n",
    ...
    fprintf(stderr,
            "hrt-m5: loader=[%p,%p) base=%p entry=%p "
            "syscalls=%zu fs=%zu\n",
    ...
    hrt_host_ui_run(stack_pointer, (void *)interpreter.entry_address);
}
