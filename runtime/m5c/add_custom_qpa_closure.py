...
PLUGIN_GUEST = Path("/opt/hrt/qt/plugins/platforms/libqhrtmac.so")
...
def main() -> int:
    ...
    if not is_elf(plugin):
        raise RuntimeError(f"custom QPA plugin is not ELF: {plugin}")
    ...
    if missing:
        raise RuntimeError(
            "unresolved hrtmac plugin dependencies: " + ", ".join(missing)
        )
    ...
    for guest, source in sorted(objects.items()):
        ...
        if destination.exists() and guest != PLUGIN_GUEST.as_posix():
            continue
    ...
    metadata = output_root / ".hrt-m5c"
    metadata.mkdir(parents=True, exist_ok=True)
    (metadata / "hrtmac-ldd.txt").write_text(ldd_output, encoding="utf-8")
    ...
    print(ldd_output)
    print(f"added {len(objects)} hrtmac QPA ELF objects")
    return 0
