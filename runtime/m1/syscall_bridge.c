...
static void sigill_handler(int signo, siginfo_t *info, void *context_pointer) {
    ...
    if (instruction[0] != 0x0f || instruction[1] != 0x0b) {
        static const char message[] = "hrt-m1: unexpected SIGILL\n";
    ...
    switch (state->__rax) {
        ...
        default: {
            static const char message[] = "hrt-m1: unsupported Linux syscall\n";
    ...
    g_in_handler = 0;
#else
#error "hrt-m1 must be built as x86_64 Mach-O"
#endif
}
