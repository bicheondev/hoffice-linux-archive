...
int main(int argc, char **argv) {
    ...
    if (program.interp_path[0] == '\0') {
        errno = 0;
        fatal("M1 program does not contain PT_INTERP");
    }
    ...
    fprintf(stderr,
            "hrt-m1: program image=[%p,%p) bias=%p entry=%p "
            "relocations=%zu patched=%zu interp=%s\n",
