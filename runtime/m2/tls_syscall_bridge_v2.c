...
static void restore_and_fail(uint64_t fs_base, const char *message,
...
static void sigill_handler(int signo, siginfo_t *info, void *context_pointer) {
    ...
    if (g_guest_fs_base != 0u && interrupted_fs != g_guest_fs_base) {
        static const char mismatch[] = "hrt-m2-tls-v2: unexpected interrupted FS base\n";
        restore_and_fail(interrupted_fs, mismatch, sizeof(mismatch) - 1u, 121);
    }
    ...
    if (instruction[0] != 0x0f || instruction[1] != 0x0b) {
        static const char unexpected[] = "hrt-m2-tls-v2: unexpected SIGILL\n";
        restore_and_fail(interrupted_fs, unexpected, sizeof(unexpected) - 1u, 124);
    }

    if (state->__rax == 158 && state->__rdi == LINUX_ARCH_SET_FS) {
        uint64_t requested = state->__rsi;
        g_guest_fs_base = requested;
        state->__rdi = requested;
        state->__rax = 0;
        state->__rcx = rip + 2u;
        state->__r11 = rip + 2u;
    ...
    switch (state->__rax) {
        ...
        case 39:
            ...
            break;
        case 158:
            if (state->__rdi == LINUX_ARCH_GET_FS && state->__rsi != 0u) {
        ...
        default: {
            static const char unsupported[] = "hrt-m2-tls-v2: unsupported syscall\n";
            restore_and_fail(interrupted_fs, unsupported,
                             sizeof(unsupported) - 1u, 126);
        }
    }
