...
int main(int argc, char **argv) {
    ...
    if (access(prepatch_marker, R_OK) != 0) {
        errno = 0;
        fatal("HWord entry gate requires a verified prepatched rootfs");
    }
    set_prepatched_code_mode(1);
    fprintf(stderr,
            "hrt-m3: verified instruction-boundary HWord rootfs enabled\n");
    ...
    if (program.interp_path[0] == '\0') {
        errno = 0;
        fatal("HWord program does not contain PT_INTERP");
    }
    ...
    install_main_entry_probe(program.entry_address);

    fprintf(stderr,
            "hrt-m3: program=%s image=[%p,%p) bias=%p entry=%p "
            "entry-probe=%p syscalls=%zu interp=%s\n",
            guest_program,
            (void *)program.image_start, (void *)program.image_end,
            (void *)program.load_bias, (void *)program.entry_address,
            (void *)g_m3_entry_probe, program.patched_syscalls,
            program.interp_path);
    fprintf(stderr,
            "hrt-m3: interpreter image=[%p,%p) base=%p entry=%p "
    ...
    if (interpreter.patched_syscalls == 0u) {
        errno = 0;
        fatal("HWord interpreter contains no trapped Linux syscalls");
    }
