#!/usr/bin/env python3
"""Resolve exact HWord provider slots 0x130/0x138 to ELF methods."""
...
METHOD_SLOTS = {"method_130": 0x130, "method_138": 0x138}
...
def virtual_address(blob: bytes, file_offset: int) -> tuple[int, dict[str, int]]:
    ...
    for index in range(phnum):
        p_type,p_flags,p_offset,p_vaddr,_paddr,p_filesz,p_memsz,p_align = \
            struct.unpack_from("<IIQQQQQQ", blob, phoff + index * phentsize)
...
def main() -> None:
    ...
    for key, slot in METHOD_SLOTS.items():
